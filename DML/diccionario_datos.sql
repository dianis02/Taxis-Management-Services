--En caso de que hubiera alguna modificacion, solo denle Execute a los archivos para poblar
--Y luego le dan Execute para obtener toda la informacion de las tablas 

--Regresa con mucho detalle lo que tiene la tabla como el tipo de variables, llaves, etc...
...
--Indica cuando fue modificado la tabla
select schema_name(tab.schema_id) as schema_name,
       tab.name as table_name, 
  ...
  order by schema_name,
        table_name
--Indica las llaves primarias y foraneas que hay en la Base de Datos y los JOINS de esta
...
--Regresa el numero de columnas de todas las tablas en la base de datos
select schema_name(tab.schema_id) as schema_name, 
       tab.name as table_name, 
       count(*) as columns
  from sys.tables as tab
